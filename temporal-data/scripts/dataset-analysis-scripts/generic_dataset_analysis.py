...
class GenericDatasetAnalyzer:
    """
    The generic DatasetAnalyzer is used to analyze the all converted datasets.

    The intended use is as follows:
    1. Create an instance of the GenericDatasetAnalyzer.
    ...
    """
    def __init__(self, dataset_filepath: str, analysis_output_path: str) -> None:
        #Load tokenizers
        ...
        def load_dataset(dataset_filepath: List[str]) -> List[dict]:
            """
            Loads the dataset from the specified filepath.

            Args:
                dataset_filepath: Filepath to the dataset files.
            ...
            with jsonlines.open(dataset_filepath, 'r') as file:
                dataset += [line for line in file]
            return dataset

        
        
        self.dataset_filepath = dataset_filepath
        self.dataset_instance = load_dataset(self.dataset_filepath)
        ...
        self.total_number_of_0sentence_input_percent = 0 #Percentage of data-elements with sentence length 0
        self.total_number_of_1sentence_input_percent = 0 #Percentage of data-elements with sentence length 1
        self.total_number_of_2sentence_input_percent = 0 #Percentage of data-elements with sentence length 2
        self.total_number_of_3sentence_input_percent = 0 #Percentage of data-elements with sentence length 3
        self.total_number_of_4sentence_input_percent = 0 #Percentage of data-elements with sentence length 4
        self.total_number_of_larger_5sentence_input = 0  #Percentage of data-elements with sentence length 5 and larger

        self.average_number_of_tokens_per_entry = 0 #Average number of tokens per data object/dataset element
        ...
        self.total_number_of_temporal_entities = 0 #Total number of temporal entities in the dataset
        self.total_number_of_timex_tokens = 0 #Total number of timex3 tokens in the dataset
        ...
        self.average_number_of_temporal_entities_per_data_object = 0 #Average number of temporal entities per data object/dataset element
        ...
        self.vocabulary_for_tempexp = set() #Contains the vocabulary for texts labeled with tempexp type "DATE", "TIME", "DURATION", "SET"
    ...
    def filter_special_tokens(self, items: List[str]) -> List[str]:
        """
    ...
    def process_dataset_entry(self, dataset_entry: dict) -> Tuple[str, List[str], List[str], List[str], List[dict], List[dict]]:
        ...
        tokens = dataset_entry["tokens"]
        entities = dataset_entry["entity"]
        sentences = self.sentence_tokenizer.tokenize(text)
        filtered_tokens = self.filter_special_tokens(tokens)
        return text, sentences, tokens, filtered_tokens, entities
    ...
    def process_dataset_instance(self, dataset_instance: List[dict]) -> List[Tuple[str, List[str], List[str], List[str], List[dict], List[dict]]]:
        ...
        for dataset_entry in dataset_instance:
            text, sentences, tokens, filtered_tokens, entities = self.process_dataset_entry(dataset_entry)
            processed_instance.append((text, sentences, tokens, filtered_tokens, entities))
    ...
    def analyze(self) -> None:
        ...
        """
        processed_dataset_instance = self.process_dataset_instance(self.dataset_instance)
        for processed_dataset_entry in processed_dataset_instance:
            #Extract information from dataset entry
            text, sentences, tokens, filtered_tokens, entities = processed_dataset_entry
            ...
            for entity in entities:
                ...
                entity_tokens = self.word_tokenizer.tokenize(entity["text"])
                entity_tokens_filtered = self.filter_special_tokens(entity_tokens)

                def add_to_temp_vocabulary(token):
                    self.vocabulary_for_tempexp.add(token.lower())

                if entity_type == "date":
                    for token in entity_tokens_filtered:
                        self.vocabulary_for_timex3_date.add(token.lower())
                        add_to_temp_vocabulary(token)
            ...
            if len(entities) > 0: data_entry_result.newline()
            data_entry_result.append(f"Number of entities: {len(entities)}")

            if len(entities) > 0:
                labeled_tokens_with_all_marked_entities = tokens.copy()
                labeled_text_with_all_marked_entities = ""
                for i, entity in enumerate(entities, start=1):
                    ...
                    labeled_tokens_with_all_marked_entities[entity_start] = "[" + labeled_tokens_with_all_marked_entities[entity_start]
                    labeled_tokens_with_all_marked_entities[entity_end] = labeled_tokens_with_all_marked_entities[entity_end] + f" ({entity_type})" + "]"

                for token in labeled_tokens_with_all_marked_entities:
                    labeled_text_with_all_marked_entities += token + " "
                data_entry_result.append(f"Annoted: {labeled_text_with_all_marked_entities}")
                data_entry_result.newline()

                for i, entity in enumerate(entities, start=1):
                    data_entry_result.append(f"({i})   Expression: '{entity['text']}'     Type: '{entity['type']}'     Start: '{entity['start']}'     End: '{entity['end']}'")
                    ...
                    labeled_tokens_with_marked_entities = tokens.copy()
                    labeled_tokens_with_marked_entities[entity_start] = "[" + labeled_tokens_with_marked_entities[entity_start]
                    labeled_tokens_with_marked_entities[entity_end] = labeled_tokens_with_marked_entities[entity_end] + "]"

                    labeled_text_with_marked_entities = ""
                    for token in labeled_tokens_with_marked_entities:
                        labeled_text_with_marked_entities += token + " "
                    data_entry_result.append(labeled_text_with_marked_entities)
                    data_entry_result.newline()
            ...
            for i, entity in enumerate(entities):
                ...
                if entity_type.strip().lower() in ["date", "time", "duration", "set", "tempexp"]:
                    self.total_number_of_temporal_entities += 1
                entity_tokens = self.word_tokenizer.tokenize(entity["text"])

                entity_tokens_filtered = self.filter_special_tokens(entity_tokens)
                self.counter_entity_timex3_types.update({entity_type})
                self.set_of_concrete_timex3_tokens.update(entity_tokens_filtered)
                self.total_number_of_timex_tokens += len(entity_tokens)
            ...
            self.average_number_of_temporal_entities_per_data_object = self.total_number_of_temporal_entities / self.total_number_of_dataobjects
            self.average_number_of_tokens_per_entry = self.total_number_of_tokens / self.total_number_of_dataobjects
        ...
        self.total_number_of_0sentence_input_percent = (self.total_number_of_0sentence_input / self.total_number_of_dataobjects) * 100
        self.total_number_of_1sentence_input_percent = (self.total_number_of_1sentence_input / self.total_number_of_dataobjects) * 100
        self.total_number_of_2sentence_input_percent = (self.total_number_of_2sentence_input / self.total_number_of_dataobjects) * 100
        self.total_number_of_3sentence_input_percent = (self.total_number_of_3sentence_input / self.total_number_of_dataobjects) * 100
        self.total_number_of_4sentence_input_percent = (self.total_number_of_4sentence_input / self.total_number_of_dataobjects) * 100
        self.total_number_of_larger_5sentence_input_percent = (self.total_number_of_larger_5sentence_input / self.total_number_of_dataobjects) * 100
    ...
    def generate_analysis_results(self, print_vocabulary: bool = True):
        ...
        ar.append(f"Dataset filepath: {self.dataset_filepath}")
        ...
        ar.newline()
        ar.append(f"Vocabulary size for tempexp: {len(self.vocabulary_for_tempexp)}")
        ar.append(f"Vocabulary for tempexp set: {self.vocabulary_for_tempexp}")
        ar.newline()
        ar.newline()
        ...
        ar.append(f"Total number of data objects with sentence length 0: {self.total_number_of_0sentence_input} ({self.total_number_of_0sentence_input_percent:.2f}%)")
        ar.append(f"Total number of data objects with sentence length 1: {self.total_number_of_1sentence_input} ({self.total_number_of_1sentence_input_percent:.2f}%)")
        ar.append(f"Total number of data objects with sentence length 2: {self.total_number_of_2sentence_input} ({self.total_number_of_2sentence_input_percent:.2f}%)")
        ar.append(f"Total number of data objects with sentence length 3: {self.total_number_of_3sentence_input} ({self.total_number_of_3sentence_input_percent:.2f}%)")
        ar.append(f"Total number of data objects with sentence length 4: {self.total_number_of_4sentence_input} ({self.total_number_of_4sentence_input_percent:.2f}%)")
        ar.append(f"Total number of data objects with sentence length 5 and larger {self.total_number_of_larger_5sentence_input} ({self.total_number_of_larger_5sentence_input_percent:.2f}%)")
        ar.append(f"Average number of tokens per sentence: {self.average_number_of_tokens_per_sentence}")
        ar.append(f"Average number of word tokens per entry: {self.average_number_of_tokens_per_entry}")
        ar.newline()
        ...
        ar.append(f"Total number of timex tokens: {self.total_number_of_timex_tokens}")
        ar.append(f"Average number of entities per data object: {self.average_number_of_entities_per_data_object}")
        ar.append(f"Average number of temporal entities per data object: {self.average_number_of_temporal_entities_per_data_object}")
        ...
        non_timex_entities = list()
        #Calculate percentages
        # for item, count in self.counter_entity_timex3_types.items():
        #     percentage = (count / self.total_number_of_entities) * 100
        #     timex_type = item.strip().lower()
        #     if "date" in timex_type or \
        #     "time" in timex_type or \
        #     "duration" in timex_type or \
        #     "set" in timex_type:
        #         ar.append(f"{item}: {percentage:.2f}%")
        #     else:
        #         non_timex_entities.append(f"{item}: {percentage:.2f}%")
        # for _ in range(len(non_timex_entities)):
        #     ar.newline()
        ar.newline()
        ar.append(f"Total number of temporal entities: {self.total_number_of_temporal_entities}")
        tempexp_percentage = (int(self.counter_entity_timex3_types['tempexp']) / self.total_number_of_temporal_entities) * 100
        ar.append(f"TEMPEXP count: {self.counter_entity_timex3_types['tempexp']} ({tempexp_percentage:.2f}%)")
        date_percentage = (int(self.counter_entity_timex3_types['date']) / self.total_number_of_temporal_entities) * 100
        ar.append(f"DATE count: {self.counter_entity_timex3_types['date']} ({date_percentage:.2f}%)")
        time_percentage = (int(self.counter_entity_timex3_types['time']) / self.total_number_of_temporal_entities) * 100
        ar.append(f"TIME count: {self.counter_entity_timex3_types['time']} ({time_percentage:.2f}%)")
        duration_percentage = (int(self.counter_entity_timex3_types['duration']) / self.total_number_of_temporal_entities) * 100
        ar.append(f"DURATION count: {self.counter_entity_timex3_types['duration']} ({duration_percentage:.2f}%)")
        set_percentage = (int(self.counter_entity_timex3_types['set']) / self.total_number_of_temporal_entities) * 100
        ar.append(f"SET count: {self.counter_entity_timex3_types['set']} ({set_percentage:.2f}%)")
        ar.newline()
        ...
        ar.append(f"Total concrete timex3 tokens: {len(self.set_of_concrete_timex3_tokens)}")
        ar.newline()
        ar.append(f"Unique timex patterns: {self.unique_timex_patterns}")
        ar.append(f"Total unique timex3 tokens: {len(self.unique_timex_patterns)}")
        ar.newline()
        ar.append(f"Non timex3 entities: {non_timex_entities}")
        for non_timex_entity in non_timex_entities:
            ar.append(non_timex_entity)
    ...
    def get_analysis_results(self):
        ...
        if self.is_analysis_result_generated:
            ...
            return "Analysis results have not been generated yet."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_jsonlines_dataset_filepath",
        "-i", 
        type = str,
        default = "../../entity/jsonlines/tweets_multi/tweets-full.jsonlines",
        help = "Requires a temporal dataset in jsonlines format, which has been previously converted with one of the conversion scripts."
    )
    ...
    parser.add_argument(
        ...
        "-o",
        type = str,
        default = "../../dataset-statistics/statistics-tweets-multi.txt",
        help = "The output filepath for the statistics file."
    )
    ...
    parser.add_argument(
        ...
        "-v",
        action = "store_true",
        help = "Wether to show analysis results in console or not."
    )
    args = parser.parse_args()


    print(f"Loading Dataset Analyzer...")
    print(f"Following arguments were passed:")
    print(f"Dataset input filepath:      {args.input_jsonlines_dataset_filepath} => {type(args.input_jsonlines_dataset_filepath)}")
    print(f"Analysis output filepath:    {args.output_filepath} => {type(args.output_filepath)}")
    print(f"Display verbose:             {args.verbose} => {type(args.verbose)}")

    dataset_analyzer = GenericDatasetAnalyzer(args.input_jsonlines_dataset_filepath, args.output_filepath)
    dataset_analyzer.analyze()
    dataset_analyzer.generate_analysis_results()
    dataset_analyzer.generate_analysis_data_entry_results()
    dataset_analyzer.save_analysis_results()

    if args.verbose:
        print(dataset_analyzer.get_analysis_results())
    print()
